...
char* per_print(Person *p, char* personbuf){
    ...
    sprintf(personbuf+strlen(personbuf), "First Name:        %s\n", p->FirstName);
    sprintf(personbuf+strlen(personbuf), "Last Name:         %s\n", p->LastName);

    int length = 60;
    //https://stackoverflow.com/questions/4841219/print-part-of-a-string-in-c
    if(strlen(p->StreetAddr) > 60){
      sprintf(personbuf+strlen(personbuf), "Address:           %.60s\n", p->StreetAddr);
    }
    else{
      sprintf(personbuf+strlen(personbuf), "Address:           %s\n", p->StreetAddr);
    }
    sprintf(personbuf+strlen(personbuf), "Zip:               %s\n", p->ZipCode);
    int height_int[2];
    int weight_int[2];
    float_to_ints(p->Height, height_int);
    float_to_ints(p->Weight, weight_int);

    sprintf(personbuf+strlen(personbuf),"Height:            %d.%02d\n",height_int[0], height_int[1]);
    sprintf(personbuf+strlen(personbuf),"Weight:            %d.%02d\n", weight_int[0], weight_int[1]);


    sprintf(personbuf+strlen(personbuf),"DOB 1/1/1900:      %ld\n", p->DBirth);
    sprintf(personbuf+strlen(personbuf), "------------------------");
